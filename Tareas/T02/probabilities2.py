...
class Probabilidades:
    ...
    def prob_contagio_paises(Pais):
        ...
        if Pais.poblacion_viva==0:
            ...
            valor=((Pais.poblacion_infectada+Pais.poblacion_muerta)*0.07)/((Pais.poblacion_viva+Pais.poblacion_infectada)*(len(conexiones_aereas)+len(conexiones_tierra)))
        if valor<1:
            prob=valor
        else:
            prob=1
    ...
    def se_puede_tierra(Pais):
        ...
        tasa_infectados=(Pais.poblacion_infectada+Pais.poblacion_muerta)/total
    ...
    def prob_descubrimiento(poblacion_infectada,visibilidad,poblacion_muerta,poblacion_incial):
        a=(Mundo.poblacion_infectada*Mundo.infeccion.visibilidad)*(Mundo.poblacion_muerta)**2/(Mundo.poblacion_incial)
        return a
