...
class Lloraton(Thread):
    ...
    def run(self):
        self.time = time.time()
        spawner = Thread(target=spawn_alumnos, args=(self.alumnos, 0.5,
                                                         self.ayudantes), daemon=True)
        spawner.start()
...
class Ayudante:
    ...
    def dar_puntaje(self, alumno):
        ...
        if give < 0:
            print("{}: {}, te voy a descontar {} por gil".format(self, alumno,
                                                                    -give))
        elif give > 0:
            print("{}: {}, solo porque me siento generoso te doy {}"
                  " decimas".format(self, alumno, give))
        else:
            print("{}: {}, no te voy a dar ni media decima jajaja >:D".format(
                        self, alumno))
